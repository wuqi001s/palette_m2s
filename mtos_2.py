...
def preprocess_image(path,size=64,gaussain_sigma=5):
...
img_64, mask_64, cond_image_64 = preprocess_image(input_image_pth, size=64,gaussain_sigma=2)
...
device = torch.device('cuda:0')
model_256.to(device)
model_256.set_new_noise_schedule(phase='test')
model_256.eval()
y_t_256 = model_256.m2s_add_noise(upsampled_output.unsqueeze(0).to("cuda:0"), noise=None, t=500).squeeze() # 添加噪声
print(y_t_256.shape)
print(img_256.shape)
#5.2 随机噪声
# y_t_256 = upsampled_output + torch.randn_like(upsampled_output)   # 添加随机噪声
# y_t_256 = y_t_256*mask_256.to(y_t_256.device)+ (1. -mask_256.to(y_t_256.device))*img_256.to(y_t_256.device)  # 将掩码应用到加噪图像上
# 保存加噪后的图像
save_tensor_as_image(y_t_256, "./result_m2s/y_t_256.jpg")  # 保存加噪后的图像
# 6. 使用256x256的模型进行推理
output_256, visuals_256 = denoise_image(model_256, y_t=y_t_256,cond_image=cond_image_256,y_0=img_256,mask=mask_256, sample_num=8, sampling_method="ddim")
# 保存256x256的输出图像
save_tensor_as_image(output_256.squeeze(), "./result_m2s/output_256.jpg")  # 保存256x256的输出图像
